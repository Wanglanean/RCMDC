function Uh = collo_solve1k(y_delta, u_exact, N, c, d, alpha, delta, x)
% This function solves a collocation method problem with a kernel function k
% using both nonlinear and linear equations.
% Inputs:
%   y_delta - Perturbed data
%   u_exact - Exact solution function
%   N - Number of intervals in the grid
%   c, d - Collocation points parameters (c is used, d is not directly used in this function)
...
for n = 1:N-1
    ...
    for l = 0:n-1
        % Compute coefficients a and b based on the kernel function
        a = c * (h*n - 2*h*l + 1);
        b = (c - 1) * (h + 2*h*l - h*n - 1);
        %a = int(k(n*h + c*h, l*h + v*h), [0, c]);  % Original intended integral for a
        %b = int(k(n*h + c*h, l*h + v*h), [c, 1]);  % Original intended integral for b
        
        % Accumulate the terms based on previous values of Uh
        m1 = m1 + a*Uh(n-l+1)*Uh(l+1) + b*Uh(n-l)*Uh(l+1);
    end
    
    % Compute the coefficient a for the current step
    a = -c * (h*n - 1);
    %a = int(k(n*h + c*h, n*h + v*h), [0, c]);  % Original intended integral for a

function Uh = collo_solve2(y_delta, u_exact, k,N, c, d, alpha, delta, x)
% This function solves a collocation problem using both nonlinear and linear equations
% for a system with two collocation points.
% Inputs:
...
a112 = (c1^2*(2*c1-3*c2))/(6*(c1-c2)^2);
a121 = a112;
a122 = c1^3/(6*(c1-c2)^2);
a211 = c2^3/(6*(c1-c2)^2);
a212 = -(c2^2*(3*c1-2*c2))/(6*(c1-c2)^2);
a221 = a212;
a222 = c2 + (-(5*c2^3)/6 + c1*c2^2)/(c1-c2)^2;
b111 = -((c1-1)*(c1^2-6*c1*c2+4*c1+6*c2^2-6*c2 + 1))/(6*(c1-c2)^2);
b112 = -((c1-1)^2*(2*c1-3*c2+1))/(6*(c1-c2)^2);
b121 = b112;
b122 = -(c1-1)^3/(6*(c1-c2)^2);
b211 = -(c2-1)^3/(6*(c1-c2)^2);
b212 = -((c2-1)^2*(2*c2-3*c1 + 1))/(6*(c1-c2)^2);
b221 = b212;
b222 = -((c2-1)*(6*c1^2-6*c1*c2-6*c1+c2^2+4*c2 + 1))/(6*(c1-c2)^2);
...
for n = 1:N-1
    ...
    for l = 1:n-1
        % Accumulate terms for m1 and m2 based on previous solutions Uh
    ...
    A(1, 1) = alpha(m*n+1) + 2*h*(a111*Uh(1, 1) + a121*Uh(1, 2));
    A(1, 2) = 2*h*(a112*Uh(1, 1) + a122*Uh(1, 2));
    A(2, 1) = 2*h*(a211*Uh(1, 1) + a221*Uh(1, 2));
    A(2, 2) = alpha(m*n+2) + 2*h*(a212*Uh(1, 1) + a222*Uh(1, 2));
    
    b(1) = y_delta(m*n+1) - h*m1 + alpha(n*m+1)*U0;
    b(2) = y_delta(m*n+2) - h*m2 + alpha(n*m+2)*U0;
    
    Uh(n+1, :) = (A\b)';  % Solve the linear system for Uh at the next time step
end

Uh = reshape(Uh', m*N, 1);  % Reshape Uh to a column vector for output
end
